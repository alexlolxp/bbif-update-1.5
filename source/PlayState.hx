...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['GET OUT WHILE YOU STILL CAN!!!', 0.2], //From 0% to 19%
		['Unfortunate...', 0.4], //From 20% to 39%
		['Yikes!', 0.5], //From 40% to 49%
		['Bruh...', 0.6], //From 50% to 59%
		['Hmm...', 0.69], //From 60% to 68%
		['Alright!', 0.7], //69%
		['Neat!', 0.8], //From 70% to 79%
	...
	public var noteLocked:Bool = false;

	public var leftnoteLocked:Bool = false;
	public var downnoteLocked:Bool = false;
	public var upnoteLocked:Bool = false;
	public var rightnoteLocked:Bool = false;
	...
	public var lockNoteLeft:LockStrumNote;
	public var lockNoteDown:LockStrumNote;
	public var lockNoteUp:LockStrumNote;
	public var lockNoteRight:LockStrumNote;
	...
	var glitchyyy:FlxSprite;
	var whiteflash:FlxSprite;
	var blackscreen:FlxSprite;
	var filename2bg:FlxSprite;
	var teleportEffect:FlxSprite;
	var teleportEffectBF:FlxSprite;
	var teleportEffect2:FlxSprite;
	var teleportEffectBF2:FlxSprite;
	...
	override public function create()
	{
		...
		glitchyyy = new FlxSprite();
		glitchyyy.frames = Paths.getSparrowAtlas('da schoolhouse/baldi/spookystatic', 'shared');
		glitchyyy.animation.addByPrefix('spooky', 'static', 24);
		glitchyyy.animation.play('spooky');
		glitchyyy.setGraphicSize(FlxG.width, FlxG.height);
		glitchyyy.screenCenter();
		glitchyyy.alpha = 0;
		glitchyyy.cameras = [camHUD];
		...
		whiteflash.setGraphicSize(FlxG.width, FlxG.height);
		...
		teleportEffect = new FlxSprite(DAD_X, DAD_Y - 250);
		teleportEffect.frames = Paths.getSparrowAtlas('da schoolhouse/baldi/teleport', 'shared');
		teleportEffect.animation.addByPrefix('effect', 'teleport', 24);
		teleportEffect.visible = false;

		teleportEffectBF = new FlxSprite(BF_X, BF_Y - 250);
		teleportEffectBF.frames = Paths.getSparrowAtlas('da schoolhouse/baldi/teleport', 'shared');
		teleportEffectBF.animation.addByPrefix('effect', 'teleport', 24);
		teleportEffectBF.visible = false;

		teleportEffect2 = new FlxSprite(DAD_X, DAD_Y - 250);
		teleportEffect2.frames = Paths.getSparrowAtlas('da schoolhouse/baldi/teleport', 'shared');
		teleportEffect2.animation.addByPrefix('effect', 'teleport', 24);
		teleportEffect2.visible = false;

		teleportEffectBF2 = new FlxSprite(BF_X, BF_Y - 250);
		teleportEffectBF2.frames = Paths.getSparrowAtlas('da schoolhouse/baldi/teleport', 'shared');
		teleportEffectBF2.animation.addByPrefix('effect', 'teleport', 24);
		teleportEffectBF2.visible = false;
		...
		healthBarBG = new AttachedSprite('healthbarbald');
		...
		healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 15), Std.int(healthBarBG.height - 10), this,
			'health', 0, 2);
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		lockNoteLeft = new LockStrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, 0, player);
		lockNoteDown = new LockStrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, 1, player);
		lockNoteUp = new LockStrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, 2, player);
		lockNoteRight = new LockStrumNote(ClientPrefs.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, 3, player);

		lockNoteLeft.postAddedToGroup();
		lockNoteDown.postAddedToGroup();
		lockNoteUp.postAddedToGroup();
		lockNoteRight.postAddedToGroup();

		lockNoteLeft.visible = false;
		lockNoteDown.visible = false;
		lockNoteUp.visible = false;
		lockNoteRight.visible = false;

		lockNoteLeft.cameras = [camHUD];
		lockNoteDown.cameras = [camHUD];
		lockNoteUp.cameras = [camHUD];
		lockNoteRight.cameras = [camHUD];
	...
	function teleportPaldo()
	{
		teleportEffect.visible = true;
		teleportEffectBF.visible = true;

		teleportEffect.animation.play('effect');
		teleportEffectBF.animation.play('effect');
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer) {
			...
			teleportEffect.animation.stop();
			teleportEffectBF.animation.stop();
		});
	}
	function teleportBack()
	{
		teleportEffect2.visible = true;
		teleportEffectBF2.visible = true;

		teleportEffect2.animation.play('effect');
		teleportEffectBF2.animation.play('effect');
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer) {
			...
			teleportEffect2.animation.stop();
			teleportEffectBF2.animation.stop();
		});
	}
	...
	var leftPresses:Int = 0;
	var upPresses:Int = 0;
	var downPresses:Int = 0;
	var rightPresses:Int = 0;

	var introLength:Float;
	var skipped = false;
	var pressedCtrl = false;

	public function skipIntro(song:String) //10290, 12200
		{
			if (!endingSong && !startingSong) 
			{
				...
				var skipTxt:FlxText = new FlxText(0, healthBarBG.y + 36, FlxG.width, "", 20);
				skipTxt.setFormat(Paths.font("comic.ttf"), 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				skipTxt.scrollFactor.set();
				skipTxt.borderSize = 1.25;
				skipTxt.text = 'Press Q to skip the intro to the song';
				skipTxt.screenCenter();
				skipTxt.alpha = 0;
				//skipTxt.visible = !ClientPrefs.hideHud;
				add(skipTxt);
	
				FlxTween.tween(skipTxt, {alpha: 1}, 0.3, {
					onComplete: function(twn:FlxTween) 
					{
						new FlxTimer().start(0.5, function(tmr:FlxTimer) {
							FlxTween.tween(skipTxt, {alpha: 0}, 0.3);
						});
					}
				});

				if (FlxG.keys.justPressed.Q && Conductor.songPosition < introLength && !skipped) 
					{
						//skipped = true;
						pressedCtrl = true;
					}
				...
				{	
					if (pressedCtrl)
					{
						...
						if (Conductor.songPosition < introLength)
						{
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song == 'Basics' || SONG.song == 'Lesson')
		{
			skipped = false;
			//skipIntro(SONG.song);
		}
		...
		if (leftPresses >= 10 && leftnoteLocked)
		{
			leftPresses = 0;
			lockNoteLeft.playAnim('release', true);
			leftnoteLocked = false;
			if (lockNoteLeft.animation.finished)
			{
				playerStrums.members[0].visible = true;
				lockNoteLeft.visible = false;
			}
		}
		else if (leftPresses >= 10)
		{
			leftPresses = 0;
		}

		if (downPresses >= 10 && downnoteLocked)
			{
				downPresses = 0;
				downnoteLocked = false;
		...
		if (upPresses >= 10 && upnoteLocked)
			{
				upPresses = 0;
				lockNoteUp.playAnim('release', true);
				upnoteLocked = false;
				if (lockNoteUp.animation.finished)
				{
					playerStrums.members[2].visible = true;
					lockNoteUp.visible = false;
				}
			}
			else if (upPresses >= 10)
			{
				upPresses = 0;
			}

		if (rightPresses >= 10 && rightnoteLocked)
			{
				rightPresses = 0;
				lockNoteRight.playAnim('release', true);
				rightnoteLocked = false;
				if (lockNoteRight.animation.finished)
				{
					playerStrums.members[3].visible = true;
					lockNoteRight.visible = false;
				}
			}
			else if (rightPresses >= 10)
			{
				rightPresses = 0;
			}*/
		...
		switch (curStage)
		{
			...
			case 'paldo':
				add(teleportEffect);
				add(teleportEffectBF);

				add(teleportEffect2);
				add(teleportEffectBF2);
			}
...
//(' + Math.floor(ratingPercent * 100) + '%)';
		if(ratingString == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Combo: ' + combo + ' | Rating: ' + ratingString;
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Combo: ' + combo + ' | Rating: ' + ratingString + '(' + Math.floor(ratingPercent * 100) + '%)';
		}
		...
		if (FlxG.keys.justPressed.F2 && !endingSong && !inCutscene) {
			persistentUpdate = false;
			paused = true;
			cancelFadeTween();
			CustomFadeTransition.nextCamera = camOther;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Teleport Back':
				teleportBack();
			case 'YCTP':
				openSubState(new ThinkSubState());
			case 'Static':
				var val1:Float = Std.parseFloat(value1);
				FlxTween.tween(glitchyyy, {alpha: 1}, val1);
			case 'remove static':
				glitchyyy.alpha = 0;
			case 'hud shake':
				var val1:Float = Std.parseFloat(value1);
				camHUD.shake(0.005, val1);
			case 'flashbang':
				var val1:Float = Std.parseFloat(value1);
				whiteflash.visible = true;
				FlxTween.tween(whiteflash, {alpha: 0}, val1,{
			...
			case 'blackscreen':
				var val1:Float = Std.parseFloat(value1);
				FlxTween.tween(blackscreen, {alpha: 1}, val1);
			case 'move dad to center':
				dad.x = 620;
			case 'move dad back':
			...
			case 'gf and bf disappear':
				gf.alpha = 0;
				boyfriend.alpha = 0;
			case 'gf and bf reappear':
				gf.alpha = 1;
				boyfriend.alpha = 1;
			case 'dad disconnect':
				dad.visible = false;
			...
			case 'ui remove':
				var val1:Float = Std.parseFloat(value1);
				FlxTween.tween(camHUD, {alpha: 0}, val1);
			case 'ui add':
				var val1:Float = Std.parseFloat(value1);
				FlxTween.tween(camHUD, {alpha: 1}, val1);
		}
		...
		if(isDad) {
			...
			switch (curStage)
			{
				...
				case 'school' | 'schoolEvil':
					...
					camFollow.y = boyfriend.getMidpoint().y - 200;
				case 'paldo':
					camFollow.x = boyfriend.getMidpoint().x - 700 + offsetX;
	...
	private function keyShit():Void
	{
				...
				if (controlArray.contains(true)) {
					for (i in 0...controlArray.length) {
						...
						if (sortedNotesList.length > 0) {
							...
							for (epicNote in sortedNotesList)
							{
								ghostMiss(controlArray[i], i, true, epicNote);
							}
		...
		playerStrums.forEach(function(spr:StrumNote)
		{
			if(controlArray[spr.ID] && spr.animation.curAnim.name != 'confirm') {
				...
				{
					case 0:
						lockNoteLeft.playAnim('pressed');
						lockNoteLeft.resetAnim = 0;
					case 1:
						lockNoteDown.playAnim('pressed');
						lockNoteDown.resetAnim = 0;
					case 2:
						lockNoteUp.playAnim('pressed');
						lockNoteUp.resetAnim = 0;
					case 3:
						lockNoteRight.playAnim('pressed');
						lockNoteRight.resetAnim = 0;
				}
			}
			...
			if(controlReleaseArray[spr.ID]) {
				...
				{
					case 0:
						lockNoteLeft.playAnim('static');
						lockNoteLeft.resetAnim = 0;
					case 1:
						lockNoteDown.playAnim('static');
						lockNoteDown.resetAnim = 0;
					case 2:
						lockNoteUp.playAnim('static');
						lockNoteUp.resetAnim = 0;
					case 3:
						lockNoteRight.playAnim('static');
						lockNoteRight.resetAnim = 0;
				}
			}
		});
	...
	function ghostMiss(statement:Bool = false, direction:Int = 0, ?ghostMiss:Bool = false, note:Note) {
		if (statement) {
			noteMissPress(direction, ghostMiss, note);
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (!noteLocked)
			...
			health -= daNote.missHealth - 0.04; //For testing purposes
	...
	function noteMissPress(direction:Int = 1, ?ghostMiss:Bool = false, note:Note):Void //You pressed a key when there was no notes to press for this key
	{
		if (!boyfriend.stunned)
		{
			if (note.noteType != 'Ruler Note' || note.noteType != 'BSOD' || note.noteType != 'Lock Note' || !noteLocked)
			{
				health -= 0.04;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Lock Note':
						...
						{
							case 0:
								lockNoteLeft.visible = true;
								lockNoteLeft.playAnim('confirm', true);
								playerStrums.members[0].visible = false;
								leftnoteLocked = true;
								noteLocked = true;
							case 1:
								lockNoteDown.visible = true;
								lockNoteDown.playAnim('confirm', true);
								playerStrums.members[1].visible = false;
								downnoteLocked = true;
								noteLocked = true;
							case 2:
								lockNoteUp.visible = true;
								lockNoteUp.playAnim('confirm', true);
								playerStrums.members[2].visible = false;
								upnoteLocked = true;
								noteLocked = true;
							case 3:
								lockNoteRight.visible = true;
								lockNoteRight.playAnim('confirm', true);
								playerStrums.members[3].visible = false;
								rightnoteLocked = true;
								noteLocked = true;
						}
				}
			...
			if (!note.isSustainNote && !noteLocked)
			{
				popUpScore(note);
				combo += 1;
			...
			playerStrums.forEach(function(spr:StrumNote) //all this for a lock note
			{
				switch (Math.abs(note.noteData))
				{
					case 0:
						if (!note.isSustainNote && !leftnoteLocked)
						{
							note.kill();
							notes.remove(note, true);
							note.destroy();
						...
						else if (leftnoteLocked)
						{
							var doKill:Bool = note.y < -note.height;
							if(ClientPrefs.downScroll) doKill = note.y > FlxG.height;
							
							lockNoteLeft.playAnim('pressed', true);
					...
					case 1:
						if (!note.isSustainNote && !downnoteLocked)
						{
							note.kill();
							notes.remove(note, true);
							note.destroy();
						...
						else if (downnoteLocked)
						{
							var doKill:Bool = note.y < -note.height;
							if(ClientPrefs.downScroll) doKill = note.y > FlxG.height;

							lockNoteDown.playAnim('pressed', true);
					...
					case 2:
						if (!note.isSustainNote && !upnoteLocked)
						{
							note.kill();
							notes.remove(note, true);
							note.destroy();
						...
						else if (upnoteLocked)
						{
							var doKill:Bool = note.y < -note.height;
							if(ClientPrefs.downScroll) doKill = note.y > FlxG.height;

							lockNoteUp.playAnim('pressed', true);
					...
					case 3:
						if (!note.isSustainNote && !rightnoteLocked)
						{
							note.kill();
							notes.remove(note, true);
							note.destroy();
						...
						else if (rightnoteLocked)
						{
							var doKill:Bool = note.y < -note.height;
							if(ClientPrefs.downScroll) doKill = note.y > FlxG.height;

							lockNoteRight.playAnim('pressed', true);
	...
	function crashLol() {
		...
		{
			theCrash.stop();
			LoadingState.loadAndSwitchState(new CrashState());
		});
...
class ThinkSubState extends MusicBeatSubstate
{
    //so far its somewhat working :)))
    
    var right:Bool = false;
    var pad:FlxSprite;
    var mathtext:FlxText;
    var numberAArray:Array<Int> = [
        0,
        1,
        2,
        3,
    ...
    var answerKey:FlxKey;

    override function create()
    {
        super.create();

        numberA = numberAArray[FlxG.random.int(0, 6)];
        numberB = numberBArray[FlxG.random.int(0, 5)];
        chosenProblem = problems[FlxG.random.int(0, 1)];

        pad = new FlxSprite(0, 0).loadGraphic(Paths.image('da schoolhouse/baldi/yctp', 'shared'));
        pad.scrollFactor.set();
        pad.setGraphicSize(Std.int(pad.width / 1.5));
        pad.screenCenter();
        add(pad);

        mathtext = new FlxText(500, 300, 0, numberA + chosenProblem + numberB, 32);
        mathtext.setFormat(Paths.font("comic.ttf"), 64, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
        mathtext.scrollFactor.set();
        mathtext.borderSize = 1.25;
        add(mathtext);
    }
    ...
    if (FlxG.keys.justPressed.ANY)
    {
        ...
        if (answer == 0){
        answerKey = FlxKey.ZERO;
        FlxG.sound.muteKeys = null;
        }
        if (answer == 1){
        answerKey = FlxKey.ONE;
        }
        if (answer == 2){
        answerKey = FlxKey.TWO;
        }
        if (answer == 3){
        answerKey = FlxKey.THREE;
        }
        if (answer == 4){
        answerKey = FlxKey.FOUR;
        }
        if (answer == 5){
        answerKey = FlxKey.FIVE;
        }
        if (answer == 6){
        answerKey = FlxKey.SIX;
        }
        if (answer == 7){
        answerKey = FlxKey.SEVEN;
        }
        if (answer == 8){
        answerKey = FlxKey.EIGHT;
        }
        if (answer == 9){
        answerKey = FlxKey.NINE;
        }
        if (answer < 0){
        answerKey = FlxKey.NINE;
        }
            if (FlxG.keys.checkStatus(answerKey, JUST_PRESSED))
            {
                right = true;
            }
                if (right = true)
